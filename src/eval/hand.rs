...
// 4 ways to choose 3 bits in a nibble and 6 ways to choose 2
fn get_suits(flags: u8, rank: u8) -> Vec<u8> {
    match flags {
...
pub fn full_house(mut flags: u64) -> Option<Showdown> {
    ...
    for r in (0..13).rev() {
        ...
        if count_bits(rflags) == 3 {
            cards.extend(get_suits(rflags, r));
            for i in 0..3 {
